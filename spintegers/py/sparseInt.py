"""Module for Sparse Integer Representation."""
...
class SparseInt(object):

    """List of set bits for sparse ints"""

    def __init__(self, value=0):
        if( isinstance(value, SparseInt)):
    ...
    def sparseMod(n,m):
        ...
        if(len(n)==0):
            return 0
        while(SparseInt.greaterThan(list(n),list(m))):
            if(len(m)==0):
                return n
            if(len(n)==0):
                return []
    ...
    def decMod(n,m):
        ...
        if(gte(factor,n)):
            return SparseInt.sparseMod(n,m)
        SparseInt.decMod(SparseInt.sparseSub(n,factor),m)
